...
export const cardsData = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(changeUserDataCard, (state, action) => {
      ...
      const newUserData = state.userData.results.map((data) =>
        data.id.value === action.payload.id.value &&
        data.id.name === action.payload.id.name &&
        data.name.title === action.payload.name.title
          ? action.payload
          : data,
      )
    ...
    .addCase(deleteUserDataCard, (state, action) => {
      ...
      const indexFirstSearch = userData.findIndex(
        (data) =>
          data.id.name === action.payload.id.name &&
          data.id.value === action.payload.id.value,
      )
